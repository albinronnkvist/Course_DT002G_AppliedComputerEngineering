...
internal sealed class TMicroserviceRepositoryName : ComponentResource, IResourceGroup
{
    public TMicroserviceRepositoryName(string resourceName, ApplicationIdentity applicationIdentity, ComponentResourceOptions options)
        : base("custom:azurenative:TMicroserviceRepositoryName", resourceName, options)
    {
        ResourceGroupType resourceGroupType = ResourceGroupType.TMicroserviceRepositoryName;
        
        var resourceGroup = new ResourceGroup(ResourceNamingConvention.GetResourceName(
            AzureResourceType.ResourceGroup, resourceGroupType.WorkloadName), options: new CustomResourceOptions
        {
            DependsOn = options.DependsOn,
            Parent = this
        });
        
        var managedIdentity = new UserAssignedIdentity(ResourceNamingConvention.GetResourceName(AzureResourceType.UserAssignedManagedIdentity, 
            resourceGroupType.WorkloadName), new UserAssignedIdentityArgs
        {
